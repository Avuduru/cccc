...
```css
.vertical .stickers-grid-canvas → position:absolute, bottom:1.5%, left:1.5%, right:1.5%, display:flex, align-items:flex-end
.vertical .sticker-item         → aspect-ratio:1/1, height:100%, max-height:100%, flex:0 1 auto
```

- **Containment**: Grid is absolutely positioned at the bottom. However, its **height is dynamically assigned via JS** (`adjustVerticalPositions()` in `ui.js`). 
- **Self-Regulating Behavior**: The JS mathematically allocates height to ensure the classifier pill always has breathing room. Because `.sticker-item` has `height: 100%` and `aspect-ratio: 1/1`, it shrinks perfectly if the JS squishes the container height! If you add 7 stickers, their width triggers `flex-shrink: 1`, which squeezes them to fit the screen, and their height identically shrinks.
...
.export-mode.vertical .stickers-grid-canvas {
    gap: 18px !important;    /* html2canvas fails on cqw */
...
.export-mode.vertical .sticker-item {
    flex-shrink: 0 !important; /* JS calculates explicit pixels */
}
```

### The html2canvas Aspect-Ratio Bug Fix
In Vertical Export, `html2canvas` has a bug where it fails to understand `aspect-ratio: 1/1` when Flexbox squishes the width using `flex-shrink`. It compresses the width but leaves the height stretched, creating ugly warped rectangles.
To fix this, **`flex-shrink` is disabled (`0`) for vertical mode**, and instead the JS Allocator (`adjustVerticalPositions`) mathematically calculates the absolute maximum width allowed per sticker and injects it as an explicit pixel `width` and `height`. This guarantees `html2canvas` always renders perfect squares.
...
1. **Do NOT set `flex-shrink: 0`** on `.export-mode.horizontal .sticker-item` — horizontal mode still relies on flex-shrink to compress items when there are many stickers.
...
2. **Keep `flex-shrink: 1` in horizontal mode** so the flexbox can compress items when the total exceeds the container width. (Vertical mode uses explicit JS overrides instead).
